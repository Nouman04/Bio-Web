...
@section('title', $guide->title . ' – Guide')
...
<div class="grid grid-cols-1 xl:grid-cols-12 gap-6 pt-4">
    ...
    <article class="xl:col-span-8 2xl:col-span-9 flex flex-col gap-6">
        <nav class="flex items-center gap-1.5 text-xs text-on-surface-variant flex-wrap">
            ...
            <span class="material-symbols-outlined" style="font-size:14px;">chevron_right</span>
            <a href="{{ route('student.chapters.guides', ['courseId' => $courseId, 'chapterId' => $chapterId]) }}" class="hover:text-primary transition-colors">Guides</a>
            <span class="material-symbols-outlined" style="font-size:14px;">chevron_right</span>
            <span class="text-on-surface truncate max-w-[14rem]">{{ $guide->title }}</span>
        </nav>
            ...
            <div class="flex justify-between items-start gap-4 flex-wrap">
                <div class="min-w-0">
                    <div class="flex items-center gap-2 mb-2 flex-wrap">
                        <span class="inline-block px-2 py-1 bg-primary-container/20 text-primary text-xs rounded">{{ $guide->type_label }}</span>
                        <span class="inline-block px-2 py-1 bg-secondary-container/20 text-secondary text-xs rounded">{{ $chapter->title }}</span>
                        @if($guide->topic)
                            <span class="inline-block px-2 py-1 bg-surface-container-highest text-on-surface-variant text-xs rounded">{{ $guide->topic->title }}</span>
                        @endif
                    </div>
                    <h1 class="text-3xl text-on-surface font-bold tracking-tight">{{ $guide->title ?: 'Untitled guide' }}</h1>
                    <p class="text-sm text-on-surface-variant mt-2">
                        {{ $guide->addedBy?->name ? $guide->addedBy->name . ' · ' : '' }}Last edited {{ $guide->updated_at?->diffForHumans() }}
                    </p>
                </div>

                <x-save-button :record="$guide" label="Save Guide"
                    class="bg-surface border border-outline-variant text-on-surface text-sm py-2 px-4 rounded-full hover:bg-surface-container-low flex items-center gap-2 shrink-0" />
            </div>
        </header>
        ...
        <div class="soft-card p-6 md:p-10 rounded-xl hover-lift">
            <x-module-progress :record="$guide">
                @if(filled($content))
                    <div class="guide-body space-y-6 text-base text-on-surface leading-relaxed
                ...
                @else
                    <p class="text-on-surface-variant text-sm">This guide has no content yet.</p>
                @endif
            </x-module-progress>
        </div>
        ...
        @if($previous || $next)
            <nav class="flex items-center justify-between gap-4">
                @if($previous)
                    <a href="{{ route('student.chapters.guides.show', ['courseId' => $courseId, 'chapterId' => $chapterId, 'guideId' => $previous->uuid]) }}"
                ...
                @if($next)
                    <a href="{{ route('student.chapters.guides.show', ['courseId' => $courseId, 'chapterId' => $chapterId, 'guideId' => $next->uuid]) }}"
    ...
    <aside class="xl:col-span-4 2xl:col-span-3 flex flex-col gap-6">
        {{-- Built from the guide's own headings, so every entry jumps somewhere
             real. Hidden entirely when the guide has no headings. --}}
        @if(count($headings) > 0)
        ...
        @if($guide->flashcards->isNotEmpty())
            <div class="soft-card p-6 rounded-xl hover-lift">
                <h3 class="text-sm font-semibold text-on-surface mb-3 flex items-center gap-2">
                    ...
                    Decks from this guide
                </h3>
                <div class="space-y-2">
                    @foreach($guide->flashcards as $deck)
