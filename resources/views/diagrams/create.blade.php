...
@section('title', 'Add New Diagram')
@section('meta-description', 'Upload and configure a new diagram asset for curriculum content or question banks.')

@section('page-title', 'Add New Diagram')
@section('page-subtitle', 'Upload and configure a new diagram asset for curriculum content or question banks.')
...
@section('content')
    ...
    <header class="flex flex-col gap-2 mb-6">
        <div class="flex items-center gap-2 text-xs font-medium text-outline dark:text-slate-400">
            ...
            <i class="fa-solid fa-chevron-right text-[10px]"></i>
            <span class="text-primary font-semibold">Add New Diagram</span>
        </div>
        <h2 class="text-2xl md:text-3xl font-bold text-on-surface dark:text-white">Add New Diagram</h2>
        <p class="text-sm text-on-surface-variant dark:text-slate-400">Upload and configure a new diagram asset for curriculum content or question banks.</p>
    </header>

    {{-- Form Layout (Bento Grid Style) --}}
    <form action="{{ route('diagrams.store') }}" method="POST" enctype="multipart/form-data" class="grid grid-cols-1 lg:grid-cols-12 gap-6">
        @csrf
        ...
        <div class="lg:col-span-8 flex flex-col gap-6">
            ...
            <div class="bg-surface-container-lowest/80 dark:bg-slate-800/80 backdrop-blur-md border border-white/40 dark:border-slate-700 shadow-sm rounded-xl p-6 flex flex-col gap-4">
                <label class="text-sm font-semibold text-on-surface dark:text-slate-200 flex items-center gap-2">
                    ...
                    Diagram Source <span class="text-error">*</span>
                </label>
                {{-- Drag and Drop Zone --}}
            ...
            <div class="bg-surface-container-lowest/80 dark:bg-slate-800/80 backdrop-blur-md border border-white/40 dark:border-slate-700 shadow-sm rounded-xl p-6 flex flex-col gap-4">
                <div class="flex flex-col gap-2">
                    ...
                    <input class="w-full bg-surface-container-lowest dark:bg-slate-900 border border-outline-variant/60 dark:border-slate-700 rounded-lg px-4 py-3 text-sm focus:border-primary focus:ring-1 focus:ring-primary outline-none transition-all shadow-[inset_0_1px_3px_rgba(0,0,0,0.05)] placeholder:text-outline/70 text-on-surface dark:text-slate-200" id="title" name="title" placeholder="Enter a descriptive title for this diagram" type="text" required/>
                </div>
        ...
        <div class="lg:col-span-4 flex flex-col gap-6">
            ...
            <div class="bg-surface-container-lowest/80 dark:bg-slate-800/80 backdrop-blur-md border border-white/40 dark:border-slate-700 shadow-sm rounded-xl p-6 flex flex-col gap-4">
                ...
                <div class="flex flex-col gap-2">
                    ...
                    <p class="text-xs font-medium text-outline dark:text-slate-500 mt-1">Linking a diagram makes it available as a resource during assessments.</p>
                </div>
            </div>
