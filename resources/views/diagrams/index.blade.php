...
@section('content')
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
        <div>
            <h2 class="text-2xl md:text-3xl font-bold text-on-surface dark:text-white">Diagrams</h2>
            <p class="text-sm text-on-surface-variant dark:text-slate-400 mt-1">Manage and organize visual diagrams and assets for course content.</p>
        </div>
        <a href="{{ route('diagrams.create') }}" class="bg-gradient-to-r from-primary to-primary-container text-white px-6 py-2.5 rounded-full text-sm font-semibold shadow-md hover:shadow-lg hover:scale-[1.02] transition-all flex items-center gap-2">
    ...
    <form action="{{ route('diagrams') }}" method="GET" class="glass-panel dark:bg-slate-800/80 rounded-2xl p-6 shadow-sm flex flex-col lg:flex-row gap-4 items-center justify-between mb-6 border-outline-variant/30 dark:border-slate-700">
        <div class="flex flex-col sm:flex-row gap-4 w-full lg:w-auto">
            ...
            <div class="relative w-full sm:w-48">
                <select name="topic" onchange="this.form.submit()" class="w-full py-2 pl-4 pr-10 bg-surface-container-lowest dark:bg-slate-900 border border-outline-variant/50 dark:border-slate-700 rounded-lg text-sm focus:border-primary focus:ring-1 focus:ring-primary shadow-inner inset text-on-surface dark:text-slate-200 appearance-none">
        ...
        <div class="flex flex-col sm:flex-row gap-2 w-full lg:w-auto">
            <div class="relative w-full sm:w-48">
                <i class="fa-regular fa-calendar absolute left-3 top-1/2 -translate-y-1/2 text-outline text-[14px] pointer-events-none z-10"></i>
