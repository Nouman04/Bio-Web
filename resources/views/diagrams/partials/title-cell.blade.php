...
@php
    ...
    $excerpt = html_entity_decode(strip_tags($diagram->content ?? ''), ENT_QUOTES | ENT_HTML5, 'UTF-8');
    $excerpt = Str::limit(trim(preg_replace('/\s+/u', ' ', $excerpt)), 60);
    $questions = $diagram->questionables_count ?? 0;
@endphp
<div class="flex flex-col gap-0.5 min-w-0">
    <span class="font-semibold text-on-surface dark:text-white truncate">{{ $diagram->title }}</span>
