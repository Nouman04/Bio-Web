...
@section('meta-description', 'Create a topic with its content and attachments.')

@section('page-title', 'Add New Topic')
@section('page-subtitle', 'Create a topic with its content and attachments.')
...
@section('content')
    ...
    <div class="flex items-center text-xs font-medium text-on-surface-variant dark:text-slate-400 gap-2 mb-6">
        ...
        <i class="fa-solid fa-chevron-right text-[10px]"></i>
        <span class="text-primary dark:text-primary-fixed-dim font-semibold">Add New</span>
    </div>

    {{-- Page Header --}}
    <div class="mb-8">
        <h2 class="text-2xl md:text-3xl font-bold text-on-surface dark:text-white">Add New Topic</h2>
        <p class="text-on-surface-variant dark:text-slate-400 mt-2">Create a topic with its content and attachments.</p>
    </div>

    <div class="glass-panel dark:bg-slate-800/80 rounded-3xl border border-outline-variant/30 dark:border-slate-700 shadow-sm">
        <form action="{{ route('topics.store') }}" method="POST" enctype="multipart/form-data" class="p-6 md:p-8 flex flex-col gap-6">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="flex flex-col gap-1.5">
                    ...
                    <select id="chapter_id" name="chapter_id" class="w-full bg-surface-container-low dark:bg-slate-900 border border-outline-variant rounded-xl py-2.5 px-4 text-sm focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none text-on-surface">
                        ...
                        <option value="ch1" {{ old('chapter_id') === 'ch1' ? 'selected' : '' }}>Chapter 1: Fundamentals</option>
                        <option value="ch2" {{ old('chapter_id') === 'ch2' ? 'selected' : '' }}>Chapter 2: Advanced Mechanics</option>
                    </select>
                </div>
                ...
                <div class="flex flex-col gap-1.5">
                    ...
                    <input id="title" name="title" type="text" value="{{ old('title') }}" class="w-full bg-surface-container-low dark:bg-slate-900 border border-outline-variant rounded-xl py-2.5 px-4 text-sm focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none text-on-surface" placeholder="Enter topic name" required>
                </div>
            </div>
            ...
            <div class="flex flex-col gap-1.5">
                ...
                <textarea id="content" name="content" data-quill data-quill-height="220px" placeholder="Enter topic content...">{{ old('content') }}</textarea>
            </div>
