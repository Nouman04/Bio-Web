...
@section('meta-description', 'Update a flashcard set and the questions it draws from.')
...
@section('content')
    ...
    <header class="h-20 bg-surface/80 dark:bg-slate-900/80 backdrop-blur-md border-b border-outline-variant/20 dark:border-slate-700 px-4 md:px-8 flex flex-col md:flex-row md:items-center justify-between shrink-0 z-30 sticky top-0 gap-2 -mx-6 md:-mx-8 lg:-mx-12 -mt-6">
        <div class="flex-1 max-w-2xl flex items-center gap-2">
            ...
            <input type="text" name="title" form="flashcard-form" value="{{ old('title', $flashcard['title']) }}" class="w-full bg-transparent border-none text-xl font-bold text-on-surface dark:text-white placeholder:text-outline focus:ring-0 px-0 h-full">
        </div>
    ...
    <div class="flex-1 flex flex-col lg:flex-row overflow-hidden -mx-6 md:-mx-8 lg:-mx-12 h-[calc(100vh-80px)]">
        ...
        <div class="w-full lg:w-[350px] xl:w-[400px] border-r border-outline-variant/20 dark:border-slate-700 flex flex-col bg-surface/50 dark:bg-slate-900/50 z-10 shrink-0 h-[40vh] lg:h-full overflow-hidden">
            <div class="p-6 border-b border-outline-variant/20 dark:border-slate-700 shrink-0 space-y-4">
                ...
                <div class="flex flex-col gap-3">
                    <select form="flashcard-form" name="chapter" class="w-full bg-white dark:bg-slate-800 border border-outline-variant/30 rounded-lg px-4 py-2.5 text-sm outline-none focus:border-primary">
                        <option {{ $flashcard['chapter'] === 'Chapter 3: The Cell' ? 'selected' : '' }}>Chapter 3: The Cell</option>
                        <option {{ $flashcard['chapter'] === 'Chapter 4: Metabolism' ? 'selected' : '' }}>Chapter 4: Metabolism</option>
                    </select>
                    <select form="flashcard-form" name="topic" class="w-full bg-white dark:bg-slate-800 border border-outline-variant/30 rounded-lg px-4 py-2.5 text-sm outline-none focus:border-primary">
                        <option value="">Select Topic...</option>
                        <option {{ $flashcard['topic'] === 'Cell Membrane' ? 'selected' : '' }}>Cell Membrane</option>
                    </select>
                    <div class="relative">
            ...
            <div class="flex-1 overflow-y-auto p-4 space-y-3">
                {{-- Questions already in this set show as selected --}}
                @foreach($flashcard['cards'] as $card)
                    <div class="bg-primary/5 border border-primary rounded-xl p-4 relative overflow-hidden">
                        ...
                        <div class="flex gap-3">
                            ...
                            <div class="pr-6">
                                <span class="inline-block px-2 py-0.5 bg-surface-variant text-on-surface-variant dark:bg-slate-700 dark:text-slate-300 text-[10px] font-bold rounded mb-2 uppercase">{{ $card['type'] }}</span>
                                <p class="text-sm text-on-surface dark:text-slate-200 opacity-80">{{ $card['question'] }}</p>
                            </div>
                        </div>
                    </div>
                @endforeach

                {{-- An available question that is not yet in the set --}}
                <div class="bg-white/80 dark:bg-slate-800 border border-outline-variant/30 rounded-xl p-4 cursor-pointer group hover:border-primary/50 hover:shadow-md transition-all relative overflow-hidden">
                    ...
                    <div class="flex gap-3">
                        ...
                        <div>
                            <span class="inline-block px-2 py-0.5 bg-surface-variant text-on-surface-variant dark:bg-slate-700 dark:text-slate-300 text-[10px] font-bold rounded mb-2 uppercase">Short Answer</span>
                            <p class="text-sm text-on-surface dark:text-slate-200 line-clamp-3">Describe the role of the smooth endoplasmic reticulum.</p>
        ...
        <div class="flex-1 flex flex-col bg-surface-container-lowest dark:bg-slate-900 relative z-0 h-[60vh] lg:h-full">
            <div class="p-6 border-b border-outline-variant/20 dark:border-slate-700 bg-surface/50 dark:bg-slate-800/50 backdrop-blur z-10 flex justify-between items-center shrink-0">
                ...
                <span class="text-xs font-semibold text-on-surface-variant bg-surface-variant dark:bg-slate-700 dark:text-slate-300 px-3 py-1 rounded-full">{{ count($flashcard['cards']) }} Cards Selected</span>
            </div>

            <form id="flashcard-form" action="{{ route('flashcards.update', $flashcard['id']) }}" method="POST" class="flex-1 overflow-y-auto p-4 md:p-8 space-y-6 pb-32">
                @csrf
                @method('PUT')
                ...
                @foreach($flashcard['cards'] as $index => $card)
                    <div class="bg-white dark:bg-slate-800 rounded-2xl p-4 md:p-6 shadow-sm border border-outline-variant/30 dark:border-slate-700 flex flex-col md:flex-row gap-4 md:gap-6 relative group hover:shadow-md transition-shadow">
                        <div class="flex flex-col items-center gap-2 md:border-r border-outline-variant/20 md:pr-4 shrink-0">
                            <span class="text-outline-variant font-bold text-xl">{{ str_pad($index + 1, 2, '0', STR_PAD_LEFT) }}</span>
                        ...
                        <div class="flex-1 flex flex-col xl:flex-row gap-6">
                            <div class="flex-1 space-y-3">
                                ...
                                <div class="p-4 bg-surface dark:bg-slate-900 rounded-xl border border-outline-variant/30 min-h-[120px]">
                                    <p class="text-sm text-on-surface dark:text-slate-300">{{ $card['question'] }}</p>
                                </div>
                            </div>
                            <div class="flex-1 space-y-3">
                                <h3 class="text-xs font-bold text-primary uppercase tracking-wider">Back (Answer/Hint)</h3>
                                <textarea name="cards[{{ $index }}][answer]" class="w-full bg-white dark:bg-slate-900 border border-outline-variant/30 rounded-xl p-4 text-sm text-on-surface dark:text-slate-300 min-h-[120px] resize-y focus:ring-1 focus:ring-primary outline-none transition-all" placeholder="Enter answer or hint...">{{ old("cards.$index.answer", $card['answer']) }}</textarea>
                            </div>
                        </div>
