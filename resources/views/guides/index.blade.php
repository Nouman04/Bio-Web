...
<style>
    ...
    #guides-table.dataTable thead th.dt-orderable-desc:hover { color: #001330; }
    ...
    #guides-table tbody tr:hover { background: rgba(0, 19, 48, 0.035); }
    .dark #guides-table tbody tr:hover { background: rgba(0, 19, 48, 0.12); }
...
@section('content')
    ...
    <div id="filterCardWrapper" class="filter-card-wrapper {{ $filtersOpen ? 'is-open' : '' }}">
        <div class="filter-card-inner">
            <div class="filter-card-panel glass-panel bg-surface-container-lowest/70 dark:bg-slate-800 rounded-2xl p-5 border border-outline-variant/30 dark:border-slate-700 shadow-sm">
                <form id="guides-filter-form" action="{{ route('guides', [$course, $chapter]) }}" method="GET">
    ...
    <div id="add-guide-modal-container" class="fixed inset-0 z-[100] flex items-center justify-center hidden" aria-modal="true" role="dialog">
        ...
        <div class="relative w-full max-w-3xl mx-4 glass-panel bg-surface-container-lowest dark:bg-slate-800 rounded-3xl shadow-2xl overflow-hidden border border-outline-variant/30 dark:border-slate-700 animate-[fadeSlideIn_0.25s_ease]">
            ...
            <form id="add-guide-form" data-ajax-form action="{{ route('guides.store', [$course, $chapter]) }}" method="POST" class="p-6 flex flex-col gap-4 max-h-[75vh] overflow-y-auto">
                @csrf
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            guidesTable = App.dataTable('#guides-table', {
                ...
                ajax: {
                    url: '{{ route('guides.data', [$course, $chapter]) }}',
                    data: (params) => {
        ...
        function openEditGuideModal(trigger) {
            ...
            form.action = `{{ url("courses/{$course->uuid}/chapters/{$chapter->uuid}/guides") }}/${id}`;
            ...
            App.request(`{{ url("courses/{$course->uuid}/chapters/{$chapter->uuid}/guides") }}/${id}/questions`)
        ...
        async function deleteGuide(trigger) {
            ...
            try {
                const payload = await App.request(`{{ url("courses/{$course->uuid}/chapters/{$chapter->uuid}/guides") }}/${trigger.dataset.id}`, { method: 'DELETE' });
