{{-- Actions column for the notes DataTable --}}
<div class="relative inline-block text-left action-dropdown">
    ...
    <div class="action-dropdown-menu hidden absolute right-0 z-20 mt-1 w-44 rounded-xl bg-surface-container-lowest dark:bg-slate-800 border border-outline-variant/30 dark:border-slate-700 shadow-lg py-1">
        <a href="{{ route('notes.show', [$courseId, $chapterId, $note]) }}" class="w-full flex items-center gap-2.5 px-4 py-2.5 text-sm text-on-surface dark:text-slate-200 hover:bg-primary/5 transition-colors">
        ...
        @can('edit note')
            <button type="button"
                ...
                data-type="{{ $note->type }}"
                data-summary-id="{{ $note->summary_id }}"
